{
  ...
  outputs =
    ...
    let
      ...
      # has to say 25. That is the newest OpenJDK nixpkgs builds itself;
      # `temurin-bin-26` is newer still but is Adoptium's prebuilt tarball
      # rather than a nixpkgs build, which is a different kind of thing to
      # depend on.
      jdk = pkgs.jdk25;
    in
    {
      devShells.${system}.default = pkgs.mkShell {
        ...
        # survive into a shell that has a different JDK on PATH, which is the
        # confusing half of a wrong-version error. Setting it here settles
        # it: mkShell's `env` is part of the derivation's environment and is
        # therefore already in place by the time that hook looks.
        #
